...
if __name__ == '__main__':
    ...
    for est_type_i, plot_dir_i in zip(args.est_types, plots_dirs):
        ...
        # concated = np.concatenate([plot_traj.accum_distances.reshape(-1, 1), plot_traj.abs_errors['abs_e_trans_vec']], axis=1)
        # df = pd.DataFrame(concated, columns=['dist', 'x', 'y', 'z'])
        # df.to_csv(plot_dir_i+'/saved_trans_errors.csv')
